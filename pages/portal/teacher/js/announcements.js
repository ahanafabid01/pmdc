...
function confirmDelete() {
    ...
    renderTable();
    closeDeleteModal();
    showToast('Announcement deleted.', 'fas fa-trash-alt');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('annFilter').addEventListener('change', () => { currentPage = 1; renderTable(); });

    // ── Modal close buttons ──
    document.getElementById('modalClose').addEventListener('click', closeModal);
    document.getElementById('modalCancel').addEventListener('click', closeModal);
    document.getElementById('annModalOverlay').addEventListener('click', e => {
        if (e.target === document.getElementById('annModalOverlay')) closeModal();
    });
